...
if sim=="RADGAL" || sim=="RADGAL_IC"|| sim=="RADGAL_Kolmog"
    x.*=n/Lbox
    y.*=n/Lbox
    z.*=n/Lbox
end 
...
@everywhere function integ_kdk(p1,dvx,pb,dt,qm,scale,ng,courant,iqm,interp_losses,z1,z2)
   ...
   p1[9]+=vp2[3]*dt/(scale)  #...drift
      ...
      if p1[3]>ng
       ...
       p1[1]=convert(Int64,trunc(rand()*(ng-1)))
       p1[2]=convert(Int64,trunc(rand()*(ng-1)))
       p1[3]=convert(Int64,trunc(rand()*(ng-1)))
...
if sim=="MAKITRA"
  ...
  #....we apply or not cosmological correction factors related to z 
    ...
    np1=size(p)
#    println("size of these local UHECRs ", np1 )
    ng=i2-i1+1
